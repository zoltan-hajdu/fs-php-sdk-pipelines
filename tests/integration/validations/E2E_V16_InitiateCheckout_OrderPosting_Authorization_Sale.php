...
class E2E_V16_InitiateCheckout_OrderPosting_Authorization_Sale
{
    public function testValidation($spreadSheet = 'TestData')
    {
        try {
            ...
            $testData = $readExcel->getTestData("$spreadSheet.xls",
                "InitiateCheckout",
                "E2E_V16_InitiateCheckout_OrderPosting_Authorization_Sale");
            ...
            $saleTestData = $readExcel->getTestData("$spreadSheet.xls",
                "Sale",
                "E2E_V16_InitiateCheckout_OrderPosting_Authorization_Sale");
            AuthorizationAPI::multiple($orderPostingResponse, $testData, $saleTestData);
        } catch (Exception $exception) {
            print $exception->getMessage();
            TestCase::assertNull($exception,
                "Exception Message at E2E_V16_InitiateCheckout_OrderPosting_Authorization_Sale");
        }
    }
}
