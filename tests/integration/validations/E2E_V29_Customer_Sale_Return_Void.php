...
class E2E_V29_Customer_Sale_Return_Void
{
    public function testValidation($spreadSheet = 'TestData')
    {
        try {
            ...
            $testData = $readExcel->getTestData("$spreadSheet.xls",
                "CustomerSearch",
                "E2E_V29_Customer_Sale_Return_Void");
            $customerSearchResponse = CustomerSearchAPI::build($testData);
            $testData = $readExcel->getTestData("$spreadSheet.xls", "Sale", "E2E_V29_Customer_Sale_Return_Void");
            $saleResponse = SaleAPI::build($customerSearchResponse, $testData);
            $testData = $readExcel->getTestData("$spreadSheet.xls", "Return", "E2E_V29_Customer_Sale_Return_Void");
            ReturnAPI::buildR($saleResponse, $testData, 0);
            $testData = $readExcel->getTestData("$spreadSheet.xls", "Void", "E2E_V29_Customer_Sale_Return_Void");
            VoidAPI::build($saleResponse, $testData);
        } catch (Exception $exception) {
            print $exception->getMessage();
            TestCase::assertNull($exception, "E2E_V29_Customer_Sale_Return_Void");
        }
    }
}
