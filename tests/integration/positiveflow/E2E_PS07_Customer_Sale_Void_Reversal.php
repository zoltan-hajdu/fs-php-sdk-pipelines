...
class E2E_PS07_Customer_Sale_Void_Reversal
{
    public static function test_E2E_PS07($spreadSheet = 'TestData')
    {
        ...
        try {
            ...
            $testData = $readExcel->getTestData("$spreadSheet.xls",
                "CustomerSearch",
                "E2E_PS07_Customer_Sale_Void_Reversal");

            $customerSearchRespose = CustomerSearchAPI::build($testData);

            $testData = $readExcel->getTestData("$spreadSheet.xls", "Sale", "E2E_PS07_Customer_Sale_Void_Reversal");

            $saleResponse = !is_null($customerSearchRespose) ? SaleAPI::build($customerSearchRespose, $testData) : null;

            $testData = $readExcel->getTestData("$spreadSheet.xls", "Void", "E2E_PS07_Customer_Sale_Void_Reversal");

            if (!is_null($saleResponse)) {
                VoidAPI::build($saleResponse, $testData);
            }

            $testData = $readExcel->getTestData("$spreadSheet.xls", "Reversal", "E2E_PS07_Customer_Sale_Void_Reversal");
            ...
            $_SESSION['stepData'][] =
                $reportHandler->getErrorData("Exception Message in E2E_PS07_Customer_Sale_Void_Reversal",
